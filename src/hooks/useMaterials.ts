  ...
  return useQuery({
    ...
    queryFn: withSessionCheck(async () => {
      ...
      const { data, error } = await supabase
        .from('materials')
        .select('id, display_name, name, unit, uses_variant, sale_price, item_type, item_code, gst_rate, hsn_code, item_classification, mappings:material_client_mappings(id, client_id, client_part_no, client_description)')
