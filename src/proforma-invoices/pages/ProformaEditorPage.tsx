...
import { ArrowLeft, Save, Send, CheckCircle, XCircle, FileCheck, Plus, Trash2, Loader2 } from 'lucide-react';
...
import { calculateTotals } from '../logic';
...
export default function ProformaEditorPage() {
  ...
  const [items, setItems] = useState<LineItem[]>([{ description: '', hsn_code: null, qty: 1, rate: 0, amount: 0 }]);
  ...
  const totals = useMemo(() => {
    ...
    if (validItems.length === 0) {
      return { subtotal: 0, cgst: 0, sgst: 0, igst: 0, total: 0, taxTotal: 0, items: [] };
    }
  ...
  const handleAddItem = () => {
    setItems(prev => [...prev, { description: '', hsn_code: null, qty: 1, rate: 0, amount: 0 }]);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        await createProforma(input);
      } else if (id) {
        await updateProforma(id, input);
      }
            ...
            <tbody>
              {items.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="pi-col-amount">
                    {formatCurrency(item.amount)}
                  </td>
                  <td className="pi-col-actions">
                    <button
                      type="button"
