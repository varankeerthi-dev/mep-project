...
import { GridMinimalDocumentWithTerms } from './GridMinimalDocumentWithTerms';

const safe = (value: unknown, fallback = '') => (typeof value === 'string' && value.trim() ? value.trim() : fallback);                                                                                      const toNum = (value: unknown): number => {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? parsed : 0;
};

export async function generateGridMinimalQuotationPdfBlobWithTerms(
  quotation: any,
  organisation: any,
  template: any,
): Promise<Blob> {
  ...
  if (quotation.terms_conditions) {
    try {
      const termsData = typeof quotation.terms_conditions === 'string' 
      ...
      termsText = String(quotation.terms_conditions);
    }
  }
