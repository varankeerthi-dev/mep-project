...
import { useAvailabilityInquiries, useCreateAvailabilityInquiry, useProcureRequisitionLines, useUpsertAvailabilityResponse, useVendors } from '../hooks/usePurchaseQueries';
            ...
            {(inq.lines || []).map((line: any) => (
              <div key={line.id} className="mt-2 border-t border-zinc-100 pt-2">
                ...
                <div className="text-xs text-zinc-500 mt-1">
                  {(line.responses || []).length === 0 ? 'No responses yet' : line.responses.map((r: any) => `${r.vendor?.company_name || 'Vendor'}: ${r.available_qty}`).join(' | ')}
                </div>
