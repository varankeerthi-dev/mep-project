  ...
  return (
    <AuthContext.Provider value={{ user, organisation, handleLogout }}>
      <div className="app-container">
        <QuickAccessBar onQuickAction={handleQuickAction} organisation={organisation} onLogout={handleLogout} />
        <Sidebar currentPath={currentPath} onNavigate={navigate} collapsed={sidebarCollapsed} onToggle={() => setSidebarCollapsed(!sidebarCollapsed)} />
    ...
    <div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
          <button className="btn btn-primary" onClick={() => onNavigate('/dc/create')}>Create New DC</button>
          <button className="btn btn-secondary" onClick={() => onNavigate('/clients/new')}>Add Client</button>
          <button className="btn btn-secondary" onClick={() => onNavigate('/store/materials')}>Add Material</button>
          <button className="btn btn-secondary" onClick={() => onNavigate('/todo')}>To Do List</button>
        </div>
      </div>
    </div>
  );
...
function TodoList() {
  ...
  return (
    <div>
      ...
      <div className="card">
        <div style={{ display: 'flex', gap: '12px', marginBottom: '20px' }}><input type="text" className="form-input" value={newTodo} onChange={e => setNewTodo(e.target.value)} placeholder="Add task..." /><button className="btn btn-primary" onClick={addTodo}>Add</button></div>
        {todos.map(todo => (<div key={todo.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', borderBottom: '1px solid var(--border-light)' }}><input type="checkbox" checked={todo.status === 'completed'} onChange={() => toggleTodo(todo.id, todo.status)} /><span style={{ flex: 1, textDecoration: todo.status === 'completed' ? 'line-through' : 'none' }}>{todo.title}</span><button className="btn btn-sm btn-secondary" onClick={() => deleteTodo(todo.id)}>Delete</button></div>))}
      </div>
    </div>
  );
}
...
function CreateClient({ onSuccess, onCancel, editMode, clientData }) {
  const [formData, setFormData] = useState(clientData || { client_name: '', address1: '', address2: '', state: '', gstin: '', contact: '', email: '', vendor_no: '', shipping_address: '', remarks: '', category: 'Active' });
    ...
    <div>
      ...
      <div className="card">
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-row">
            <div className="form-group"><label className="form-label">Contact Number</label><input type="text" className="form-input" value={formData.contact} onChange={e => setFormData({...formData, contact: e.target.value})} /></div>
          ...
          <div className="form-row">
            <div className="form-group"><label className="form-label">Address 1</label><input type="text" className="form-input" value={formData.address1} onChange={e => setFormData({...formData, address1: e.target.value})} /></div>
            <div className="form-group"><label className="form-label">Address 2</label><input type="text" className="form-input" value={formData.address2} onChange={e => setFormData({...formData, address2: e.target.value})} /></div>
          </div>
          <div className="form-row">
            <div className="form-group">
            ...
            <div className="form-group"><label className="form-label">Shipping Address</label><input type="text" className="form-input" value={formData.shipping_address} onChange={e => setFormData({...formData, shipping_address: e.target.value})} /></div>
          </div>
