...
type DiscountApplyMode = 'skip' | 'override';
...
export function BOQ() {
  ...
  const [items, setItems] = useState<Record<string, BoqRow[]>>({});
  ...
  const [materialSearchActive, setMaterialSearchActive] = useState<number | null>(null);
  ...
  const [pendingDiscountChange, setPendingDiscountChange] = useState<PendingDiscountChange | null>(null);
  const [discountApplyMode, setDiscountApplyMode] = useState<DiscountApplyMode>('skip');
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (parsed?.orientation) setExportOrientation(parsed.orientation as 'landscape' | 'portrait');
      } catch {}
    }
  }, [boqData.boqNo]);
  ...
  useEffect(() => {
    const colSelection: Record<string, boolean> = {};
      ...
      if (editId) {
        ...
        if (header) {
          ...
          const loadedSheets = (header.sheets || [])
            .slice()
            .sort((a: any, b: any) => (a.sheet_order || 0) - (b.sheet_order || 0))
            .map((s: any) => ({
              id: s.id,
              name: s.sheet_name,
          ...
          const itemsMap: Record<string, BoqRow[]> = {};
          (header.sheets || []).forEach((sheet: any) => {
  ...
  const fetchClientDiscounts = useCallback(async (clientId: string) => {
    if (!clientId) return;
    try {
  ...
  const fetchBoqVariantDiscounts = useCallback(async (variantId: string) => {
    if (!variantId) {
  ...
  const computeEffectiveDiscount = useCallback((row: BoqRow) => {
  ...
  const handleVariantChange = useCallback((index: number, newVariantId: string) => {
  ...
  const handleDefaultVariantChange = useCallback((newVariantId: string) => {
    const currentItems = items[activeSheetId] || [];
    ...
    sheets.forEach((sheet, sheetIndex) => {
      ...
      sheetItems.forEach(row => {
        if (row.isHeaderRow) {
          ...
          const dataRow = visibleCols.map((col: ColumnSetting) => {
  ...
  const handleSheetDragStart = (e: any, sheetId: string) => {
    setDragSheetId(sheetId);
    e.dataTransfer.effectAllowed = 'move';
  };

  const handleSheetDragOver = (e: any) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
  };

  const handleSheetDrop = (e: any, targetSheetId: string) => {
    e.preventDefault();
  ...
  const handleRowDragStart = (e: any, index: number) => {
    setDragRowIndex(index);
    e.dataTransfer.effectAllowed = 'move';
  };

  const handleRowDragOver = (e: any) => {
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';
  };

  const handleRowDrop = (e: any, targetIndex: number) => {
    e.preventDefault();
  ...
  const handleMaterialSearch = (index: number, searchTerm: string) => {
  ...
  const handleKeyDown = (e: any, rowIndex: number, colKey: string) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const currentItems = items[activeSheetId] || [];
  ...
  const excelStyle: CSSProperties & Record<string, string> = {
                ...
                <div className="flex space-x-4">
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setExportOrientation(e.target.value as 'landscape' | 'portrait')}
            ...
            <div className="space-y-2 mb-4">
              <label className="flex items-center space-x-2 p-2 border rounded cursor-pointer hover:bg-slate-50">
                <input
                  ...
                  onChange={(e) => setDiscountApplyMode(e.target.value as DiscountApplyMode)}
                  className="text-green-600 focus:ring-green-500"
                />
