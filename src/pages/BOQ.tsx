...
export function BOQ() {
  ...
  const getPrice = useCallback(async (itemId, variantId, make) => {
    ...
    return queryClient.fetchQuery({
      queryKey: ['boqPrice', itemId, variantId || null, make || ''],
      staleTime: 5 * 60 * 1000,
      queryFn: async () => {
        try {
          if (variantId && make) {
            const priceWithMake = await timedSupabaseQuery(
              supabase
                ...
                .eq('is_active', true)
                .maybeSingle(),
              'BOQ make-based price',
            );

            if (priceWithMake?.sale_price) return priceWithMake.sale_price;
          }

          if (variantId) {
            const priceWithVariant = await timedSupabaseQuery(
              supabase
                .from('item_variant_pricing')
                .select('sale_price')
                .eq('item_id', itemId)
            ...
            if (priceWithVariant?.sale_price) return priceWithVariant.sale_price;
          }
        } catch (error) {
          console.error('Error loading BOQ item price:', error);
        }

        const material = materials.find((entry) => entry.id === itemId);
