  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-zinc-900">Job Cards</h1>
          <p className="text-zinc-500 mt-1">Track material issuance and production</p>
        </div>
        <button
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-zinc-200">
                <th className="text-left px-6 py-4 text-sm font-medium text-zinc-500">Job Card No</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-zinc-500">Product</th>
                <th className="text-left px-6 py-4 text-sm font-medium text-zinc-500">Planned Qty</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                pagedData.map((jc) => (
                  ...
                  >
                    <td className="px-6 py-4 font-medium text-zinc-900">{jc.job_card_no}</td>
                    <td className="px-6 py-4 text-zinc-700">{jc.product_name}</td>
                    <td className="px-6 py-4 text-zinc-700">{jc.planned_qty} {jc.output_unit}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${statusColors[jc.status] || 'bg-zinc-100 text-zinc-600'}`}>
                        {jc.status.replace('_', ' ')}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="relative inline-block" ref={openMenuId === jc.id ? menuRef : undefined}>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                          ...
                          <div className="absolute right-0 mt-1 w-48 bg-white border border-zinc-200 rounded-lg shadow-lg z-10 py-1">
                            <button
                              onClick={(e) => {
                                e.stopPropagation();
                                setOpenMenuId(null);
                                onNavigate(`/manufacturing/job-cards/${jc.id}`);
                              }}
                            ...
                            {jc.status === 'issued' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setOpenMenuId(null);
                                  onNavigate(`/manufacturing/production/create?jobCard=${jc.id}`);
                                }}
                                className="w-full text-left px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50"
                              >
                                Record Production
                            ...
                            {jc.status === 'draft' && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setOpenMenuId(null);
                                  onNavigate(`/manufacturing/job-cards/create?bom=${jc.bom_id}`);
                                }}
                                className="w-full text-left px-4 py-2 text-sm text-zinc-700 hover:bg-zinc-50"
                              >
                                Create Similar
