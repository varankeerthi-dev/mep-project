...
export default function TransactionNumberSeries() {
  ...
  const generatePreview = (prefix: string, startNumber: number, suffix: string, padding: number | string = 4) => {
    const num = String(startNumber || 1).padStart(parseInt(padding) || 4, '0');
    return `${prefix || ''}${num}${suffix || ''}`;
  };

  const getPreviewForDocType = (seriesData: any, docType: string) => {
...
function SeriesModal({ series, onClose, onSave, generatePreview }: SeriesModalProps) {
  ...
  useEffect(() => {
    if (!formData.configs || Object.keys(formData.configs).length === 0) {
      ...
      setFormData((prev: any) => ({ ...prev, configs: initialConfigs }));
    }
  }, []);

  const updateConfig = (docType: string, field: string, value: any) => {
