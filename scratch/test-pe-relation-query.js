...
async function test() {
  const { data, error } = await supabase
    ...
    .select('*, job_cards(job_card_no, product_name)')
    .limit(3);

  if (error) {
    console.error('Error fetching production entries with job_cards:', error);
  } else {
    console.log('Query success! Sample rows:', data);
  }
}
