...
export default function ClientLookup() {
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      {/* Selectors Panel */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-5 bg-white/60 dark:bg-zinc-900/40 backdrop-blur-md border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm">
        <div className="space-y-2">
          <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500">Client</label>
          {isClientsLoading ? (
            <div className="h-10 bg-zinc-100 dark:bg-zinc-800 animate-pulse rounded-lg" />
          ) : (
            <select
              className="w-full h-10 px-3 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/25 transition-all text-zinc-900 dark:text-zinc-100"
              value={selectedClientId}
              onChange={(e) => setSelectedClientId(e.target.value)}
            >
              <option value="">-- Select Client --</option>
              {clients.map(c => (
                <option key={c.id} value={c.id}>{c.client_name} ({c.client_id})</option>
              ))}
            </select>
          )}
        </div>
        ...
        <div className="space-y-2">
          <label className="block text-xs font-semibold uppercase tracking-wider text-zinc-500">Project / Site</label>
          {isProjectsLoading ? (
            <div className="h-10 bg-zinc-100 dark:bg-zinc-800 animate-pulse rounded-lg" />
          ) : (
            <select
              className="w-full h-10 px-3 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/25 transition-all text-zinc-900 dark:text-zinc-100 disabled:opacity-50"
      ...
      {!selectedProjectId ? (
        <div className="p-12 text-center border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl text-zinc-400">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* 1. Dispatch Blocker status */}
            <div className="p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm space-y-4">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-zinc-400">1. Dispatch & Order Blocker Check</h2>
            ...
            <div className="p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm space-y-4">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-zinc-400">2. Scope & Quotation Verification</h2>
              ...
              <form onSubmit={handleScopeSearch} className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full h-9 pl-9 pr-3 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/25 transition-all text-zinc-900 dark:text-zinc-100"
                    value={scopeKeyword}
                ...
                <button
                  ...
                  className="px-4 h-9 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium transition-colors shadow-sm disabled:opacity-50 flex items-center gap-1.5 cursor-pointer"
                >
          ...
          {/* Logging Form Panel */}
          <div className="p-5 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm space-y-4 h-fit">
            <div className="border-b border-zinc-100 dark:border-zinc-850 pb-2">
              <h2 className="text-sm font-semibold uppercase tracking-wider text-zinc-400">3. Log Client Interaction</h2>
              <p className="text-[10px] text-zinc-400 mt-0.5">Save resolving actions and route notifications immediately.</p>
            </div>

            <div className="space-y-3">
              {/* Category */}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-500">Party Type</label>
                    ...
                    <button
                      ...
                      className={`flex-1 py-1.5 text-xs font-semibold rounded-lg border transition-all cursor-pointer ${
                        callCategory === c
                          ...
                          : 'bg-white border-zinc-200 text-zinc-600 hover:bg-zinc-50 dark:bg-zinc-950 dark:border-zinc-800'
                      }`}
                    >
                      {c}
                    </button>
              ...
              {/* Priority */}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-500">Call Priority</label>
                <div className="flex gap-2">
                  {['Normal', 'Urgent'].map(p => (
                    <button
                      key={p}
              ...
              {/* Notes */}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-500">Call Discussion Notes</label>
                <textarea
                  placeholder="Enter details of client request or discussion..."
                  className="w-full min-h-[70px] p-2.5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/25 transition-all text-zinc-900 dark:text-zinc-100"
                  value={callNotes}
              ...
              <div className="p-3 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-xl space-y-2">
                ...
                {requestedAdditionalScope && (
                  <textarea
                    ...
                    className="w-full min-h-[50px] p-2 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/25 transition-all text-zinc-900 dark:text-zinc-100"
              ...
              {/* Routing Assignee */}
              <div className="space-y-1">
                <label className="text-xs font-semibold text-zinc-500">Route & Assign Follow-up To</label>
                <select
                  className="w-full h-9 px-2.5 border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/25 transition-all text-zinc-900 dark:text-zinc-100"
                  value={assignedToId}
              ...
              <button
                ...
                className="w-full py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-all shadow-sm hover:shadow flex justify-center items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
