...
export function WorkOrderDetailView({ workOrderId, onNavigate }: WorkOrderDetailViewProps) {
  ...
  const { data: workOrder, isLoading: woLoading } = useQuery({
    queryKey: ['subcontractors-v2', 'work-order-detail', workOrderId],
    queryFn: async () => {
  ...
  const { data: payments } = useQuery({
    queryKey: ['subcontractors-v2', 'work-order-payments', workOrderId],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  if (!workOrder) {
    return (
      <div className="p-6 text-center text-red-600" style={{ fontFamily: 'Inter, sans-serif' }}>
        ...
        <button onClick={() => onNavigate?.('/subcontractors-v2/workorders')} className="ml-4 px-3 py-1 border border-black">Go Back</button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white" style={{ fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="sticky top-0 bg-white border-b-2 border-black px-6 py-4 z-50">
        <div className="flex justify-between items-start">
          <div>
            <button
              onClick={() => onNavigate?.('/subcontractors-v2/workorders')}
      ...
      {showFinalPaymentModal && (
        <FinalPaymentModal
          ...
          onSuccess={(retentionData) => {
            ...
            queryClient.invalidateQueries({ queryKey: ['subcontractors-v2', 'work-order-payments', workOrderId] });
            queryClient.invalidateQueries({ queryKey: ['subcontractors-v2', 'work-order-detail', workOrderId] });
            alert(retentionData 
