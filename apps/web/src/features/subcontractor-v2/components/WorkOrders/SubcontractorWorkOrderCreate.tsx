...
export default function SubcontractorWorkOrderCreate({ onNavigate }: { onNavigate?: (path: string) => void }) {
  ...
  useEffect(() => {
    const calcResult = calculateFinance({
      ...
      cgstPercent: formData.cgst_percent,
      sgstPercent: formData.sgst_percent,
      igstPercent: formData.igst_percent,
      tdsPercent: formData.tds_percent,
      advancePercent: formData.advance_percent,
      retentionHeld: formData.retention_held,
      retentionPercent: formData.retention_percent,
    });
    ...
    setFormData((prev) => ({
      ...
      tds_amount: calcResult.tdsAmount,
      total_amount: calcResult.totalAmount,
      advance_amount: calcResult.advanceAmount,
      retention_amount: calcResult.retentionAmount,
    }));
  }, [
