...
const BOMHeaderSchema = z.object({
  ...
  product_id: emptyToNull(z.string().uuid().nullable().optional()),
...
const BOMItemSchema = z.object({
  ...
  company_variant_id: emptyToNull(z.string().uuid().nullable().optional()),
  variant_name: z.string().nullable().optional(),
  make: z.string().nullable().optional(),
  lead_time_days: z.number().int().nonnegative().default(0),
  bom_level: z.number().int().nonnegative().default(0).optional(),
  parent_material_id: emptyToNull(z.string().uuid().nullable().optional()),
  custom_attributes: z.record(z.string(), z.any()).default({}),
  unit_cost: z.number().nonnegative().default(0).optional(),
  sequence_no: z.number().int().nonnegative().default(0).optional(),
  work_center_id: emptyToNull(z.string().uuid().nullable().optional()),
  is_critical: z.boolean().default(false).optional(),
  alternate_material_id: emptyToNull(z.string().uuid().nullable().optional()),
  drawing_reference: z.string().nullable().optional(),
