...
export function useMaterialForm() {
  ...
  const handleSubmit = async (e: any, config: {
    ...
    const materialData = {
      ...
      unit: formData.unit,
      sale_price: formData.uses_variant ? null : (formData.sale_price ? parseFloat(formData.sale_price as string) : null),
      purchase_price: formData.uses_variant ? null : (formData.purchase_price ? parseFloat(formData.purchase_price as string) : null),
