...
export class ApprovalWorkflowEngine {
    ...
    try {
      ...
      switch (approval.approval_type) {
        case 'PURCHASE_ORDER':
          ...
          break;
        case 'SALES_ORDER':
          await this.triggerSalesOrderWorkflows(approval);
          break;
        case 'JOB_CARD':
          await this.triggerJobCardWorkflows(approval);
          break;
        case 'WORK_ORDER':
  ...
  private static async triggerSalesOrderWorkflows(approval: Approval): Promise<void> {
    try {
      console.log('Triggering sales order workflows:', approval.id);
    } catch (error) {
      console.error('Error triggering sales order workflows:', error);
    }
  }

  private static async triggerJobCardWorkflows(approval: Approval): Promise<void> {
    try {
      console.log('Triggering job card workflows:', approval.id);
    } catch (error) {
      console.error('Error triggering job card workflows:', error);
    }
  }
