...
const SOURCES: LeadSource[] = [
  ...
  'Existing Client', 'Existing Client Ex-Employee', 'LinkedIn', 'Advertisement', 'Walk-in',
...
export function LeadCaptureModal({
  ...
  const [scheduleSiteVisit, setScheduleSiteVisit] = useState(false);
  const [svVisitDate, setSvVisitDate] = useState(() => {
    const d = new Date(); d.setDate(d.getDate() + 1); return d.toISOString().split('T')[0];
  });
  const [svEngineer, setSvEngineer] = useState('');
  const [svContactPerson, setSvContactPerson] = useState('');
  const [svContactPhone, setSvContactPhone] = useState('');
  ...
  const handleSubmit = () => {
    ...
    createLead.mutate(input, {
      onSuccess: async (saved) => {
        if (scheduleSiteVisit && saved?.id) {
          try {
            const { error: svError } = await supabase.from('site_visits').insert({
              visit_date: svVisitDate,
              engineer: svEngineer || null,
            ...
            if (svError) console.error('Site visit creation failed:', svError);
          } catch (e) {
            console.error('Site visit creation error:', e);
          }
        }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 max-h-[80vh] overflow-y-auto">
        <div style={cardBody}>
          ...
          {/* Site Visit checkbox */}
          <div style={{ marginTop: '12px', borderTop: '1px solid #e4e4e7', paddingTop: '12px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '12px', fontWeight: 600, color: '#374151' }}>
              <input
                type="checkbox"
                checked={scheduleSiteVisit}
                onChange={(e) => setScheduleSiteVisit(e.target.checked)}
            ...
            {scheduleSiteVisit && (
              <div style={{ marginTop: '10px', padding: '12px', background: '#f0f7ff', borderRadius: '6px', border: '1px solid #bfdbfe' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <div>
                    <div style={{ fontSize: '10px', fontWeight: 600, color: '#374151', marginBottom: '3px' }}>Visit Date *</div>
                    <input type="date" value={svVisitDate} onChange={(e) => setSvVisitDate(e.target.value)}
                      style={{ width: '100%', padding: '6px 8px', fontSize: '12px', border: '1px solid #d1d5db', borderRadius: '4px', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', fontWeight: 600, color: '#374151', marginBottom: '3px' }}>Engineer Name</div>
                    <input type="text" value={svEngineer} onChange={(e) => setSvEngineer(e.target.value)} placeholder="Engineer name"
                      style={{ width: '100%', padding: '6px 8px', fontSize: '12px', border: '1px solid #d1d5db', borderRadius: '4px', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', fontWeight: 600, color: '#374151', marginBottom: '3px' }}>Contact Person</div>
                    <input type="text" value={svContactPerson} onChange={(e) => setSvContactPerson(e.target.value)} placeholder="Contact person name"
                      style={{ width: '100%', padding: '6px 8px', fontSize: '12px', border: '1px solid #d1d5db', borderRadius: '4px', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <div style={{ fontSize: '10px', fontWeight: 600, color: '#374151', marginBottom: '3px' }}>Contact Phone</div>
                    <input type="text" value={svContactPhone} onChange={(e) => setSvContactPhone(e.target.value)} placeholder="Phone number"
                      style={{ width: '100%', padding: '6px 8px', fontSize: '12px', border: '1px solid #d1d5db', borderRadius: '4px', boxSizing: 'border-box' }} />
                  </div>
                </div>
              </div>
