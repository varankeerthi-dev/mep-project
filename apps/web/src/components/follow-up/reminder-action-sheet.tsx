...
export function InvoiceDetailPanel({
  ...
  if (!invoice || !meta) {
    return (
      <aside className="hidden w-80 shrink-0 rounded-lg border border-slate-200 bg-slate-50/50 p-4 xl:block">
        <p className="text-sm text-slate-500">Select an invoice to view escalation detail.</p>
      </aside>
    );
  }

  return (
    <aside className="hidden w-80 shrink-0 flex-col rounded-lg border border-slate-200 bg-white xl:flex">
      <div className="border-b border-slate-100 px-4 py-3">
        <p className="text-[10px] font-semibold uppercase text-slate-500">Detail</p>
        <p className="font-mono text-sm font-semibold text-slate-900">{invoice.invoice_no}</p>
        <div className="mt-2">
      ...
      <div className="flex-1 space-y-3 overflow-auto px-4 py-3 text-xs">
        ...
        <div>
          ...
          <p className="text-slate-800">{invoice.project_name}</p>
        </div>
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-3">
          ...
          <p className="mt-1 text-slate-800 leading-relaxed">{meta.recommendedAction}</p>
        </div>
      ...
      <div className="sticky bottom-0 flex gap-2 border-t border-slate-100 bg-white p-3">
        <button
          ...
          className="flex-1 rounded-lg bg-blue-600 px-3 py-2 text-xs font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"
        >
          Send reminder
        </button>
        <button
