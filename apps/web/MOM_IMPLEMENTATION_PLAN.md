...
**Gate:** Product owner confirms that the charter accurately reflects the intended scope and boundaries.

## Phase 2 — Repository inventory and migration checklist

**Objective:** Map existing Meetings/MOM code, data, and dependencies before making changes.

**Inventory tasks:**

- Catalog all meeting-related routes, components, hooks, API functions, and SQL files.
- Identify legacy vs. canonical implementations and their dependencies.
- Document existing RLS policies, indexes, and constraints.
- List existing tests and their coverage gaps.
- Identify external dependencies (storage, notifications, search services).

**Deliverables:** Dependency map, migration checklist, and risk register.

**Gate:** Inventory is complete and reviewed by a senior engineer.

## Phase 3 — Normalize the domain model

**Objective:** Extend the schema with explicit topics, decisions, links, versions, and audit events.

**Schema changes:**

- Add `meeting_topics` table for structured discussion points.
- Add `meeting_decisions` table for formal decisions with rationale and owners.
- Add `meeting_links` table for explicit relationships to projects, tasks, milestones, and sites.
- Add `meeting_versions` table for immutable MOM snapshots.
- Add `meeting_audit_events` table for change tracking.
- Extend `meeting_action_items` with optional `topic_id` and `decision_id` foreign keys.
- Add `parent_meeting_id` to `meetings` for series/continuation support.

**Deliverables:** ERD, migration SQL, type definitions, and unit tests.

**Gate:** Schema migration passes linting and basic integration tests.

## Phase 4 — Define security, versioning, and audit rules

**Objective:** Specify permissions, immutability guarantees, and audit behavior before implementing logic.

**Security rules:**

- Define organization-scoped RLS for all new tables.
- Specify who can create/edit/finalize MOMs and who can only view.
- Define external sharing permissions and expiration hooks.

**Versioning rules:**

- Finalized MOMs cannot be edited; amendments create new versions.
- Version records include the parent version, amendment reason, and approver.
- Historical relationships are preserved even when source records are archived.

**Audit rules:**

- All CRUD operations on meetings, topics, decisions, and action items are logged.
- Audit events include user, timestamp, action, and affected record IDs.
- Audit trail is queryable for compliance and debugging.

**Deliverables:** Security specification, versioning workflow, and audit schema.

**Gate:** Security and versioning specifications are approved by a security reviewer.

## Phase 5 — Preserve and normalize existing data

**Objective:** Migrate existing meetings to the new schema without data loss or downtime.

**Migration tasks:**

- Back up existing meetings, minutes, attendees, and action items.
- Run the foundation migration script idempotently.
- Backfill topics and decisions from existing minutes where possible.
- Preserve existing action-item-to-task links in the new link model.
- Validate that all historical data is accessible after migration.

**Deliverables:** Migration script, backfill procedures, and validation report.

**Gate:** Data migration is validated on a staging environment.

## Phase 6 — Deliver the core MOM editor with discussion, attendees, and draft save

**Objective:** Ship a working editor that covers the current feature set with improved reliability.

**Editor sections:**

- Meeting overview (client, project, date/time, location, type, tags).
- Attendees with internal/external distinction and presence tracking.
- Structured discussion topics with notes and status.
- Draft autosave with explicit save state and error recovery.
- Basic validation and error messaging.

**Behavior improvements:**

- Persist attendees during creation.
- Show save status (saving/saved/failed) with retry.
- Warn before leaving with unsaved changes.
- Reconcile attendee deletions on save.

**Deliverables:** Enhanced editor components, updated API functions, and integration tests.

**Gate:** Editor passes manual testing of create, edit, save, and attendee management.

## Phase 7 — Implement structured decisions and accountable action items

**Objective:** Turn meeting notes into decisions and follow-up work.

**Decisions UI:**

- Add decision title, decision text, rationale, owner, and status.
- Support proposed, confirmed, superseded, and rejected states.
- Allow linking decisions to specific discussion topics.
- Show decision history and amendment trail.
