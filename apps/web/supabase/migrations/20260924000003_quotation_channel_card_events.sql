-- Extends post_quotation_channel_card with rejected/returned events so
-- channel-side decisions are narrated in the project channel.
-- Re-runnable (CREATE OR REPLACE). Run in Supabase Dashboard SQL Editor.
...
BEGIN
  ...
  IF p_event NOT IN ('created', 'submitted', 'approved', 'rejected', 'returned') THEN RAISE EXCEPTION 'invalid_event'; END IF;
  ...
  ELSIF p_event = 'rejected' THEN
    v_content := 'Quotation ' || v_q.quotation_no || ' rejected - see approval comments';
  ELSE
    v_content := 'Quotation ' || v_q.quotation_no || ' returned for revision - see approval comments';
  END IF;
