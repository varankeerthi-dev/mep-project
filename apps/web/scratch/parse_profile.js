...
try {
  ...
  console.log("--- FOUND TWEETS ON PROFILE ---");
  
  // Sort tweets by ID or created_at
  const sortedTweets = Object.values(tweets).sort((a, b) => {
    return new Date(b.created_at) - new Date(a.created_at);
  });
  
  for (const tweet of sortedTweets) {
    const user = users[tweet.user] || {};
    console.log(`\n[Tweet ID: ${tweet.id_str}] by @${user.screen_name} (${user.name}) at ${tweet.created_at}`);
