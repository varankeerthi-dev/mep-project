...
function parseProformaRecord(row: any): ProformaWithRelations {
  ...
  const base = ProformaSchema.parse({
    ...
    render_as_tax_invoice: row.render_as_tax_invoice ?? false,
...
function cleanEmptyString<T>(val: T): T | null {
...
function buildProformaPayload(proforma: Proforma): {
  ...
  const items = proforma.items.map((item, idx) => {
    ...
    return {
      ...
      item_id: cleanEmptyString(item.item_id),
      variant_id: cleanEmptyString(item.variant_id),
      discount_category_id: cleanEmptyString(item.discount_category_id),
      make: cleanEmptyString(item.make),
      variant: cleanEmptyString(item.variant),
      unit: cleanEmptyString(item.unit),
  ...
  const proformaRow: Record<string, unknown> = {
    ...
    company_state: cleanEmptyString(proforma.company_state),
    client_state: cleanEmptyString(proforma.client_state),
    valid_until: cleanEmptyString(proforma.valid_until),
    accepted_at: cleanEmptyString(proforma.accepted_at),
    source_type: proforma.source_type,
    source_id: cleanEmptyString(proforma.source_id),
    converted_invoice_id: cleanEmptyString(proforma.converted_invoice_id),
    po_number: cleanEmptyString(proforma.po_number),
    po_date: cleanEmptyString(proforma.po_date),
    template_id: cleanEmptyString(proforma.template_id),
    authorized_signatory_id: cleanEmptyString(proforma.authorized_signatory_id),
    notes: cleanEmptyString(proforma.notes),
    terms: cleanEmptyString(proforma.terms),
    payment_terms: cleanEmptyString(proforma.payment_terms),
    render_as_tax_invoice: proforma.render_as_tax_invoice ?? false,
  };
  ...
  return {
    ...
    itemRows: items.map((item, index) => ({
      description: item.description,
      hsn_code: cleanEmptyString(item.hsn_code),
      qty: item.qty,
      rate: item.rate,
      amount: item.amount,
      discount_percent: item.discount_percent ?? 0,
