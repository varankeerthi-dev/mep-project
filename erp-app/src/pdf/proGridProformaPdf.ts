...
/**
 * Proforma Invoice document with strict label–value grids (A4).
 * Same format as Classic Quotation Template but with Proforma labels.
 */
export function generateProGridProformaPdf(data: Record<string, unknown>, organisation: Org, templateSettings?: TemplateSettings): jsPDF {
  ...
  const hl = {
    ...
    payment: (headerLabels.payment as string) || 'Payment Terms',
  ...
  y = appendLabelValueGrid(doc, y, [
    ['Bill to — Name', String(client.client_name || client.name || '—'), 'Ship to — Name', String(client.client_name || client.name || '—')],
    ['Bill to — Address', billAddr || '—', 'Ship to — Address', shipAddr || '—'],
    ['Buyer GSTIN', String(client.gst_number || client.gstin || '—'), 'Ship to GSTIN', String(data.ship_to_gstin || client.gst_number || client.gstin || '—')],
    ['State', String(data.client_state || client.state || '—'), 'Project / Site', String(data.project || '—')],
  ]);

  const isInterState =
    data.client_state &&
    organisation.state &&
    String(data.client_state).trim().toLowerCase() !== String(organisation.state).trim().toLowerCase();
  ...
  autoTable(doc, {
    ...
    body: items.map((item) => {
      ...
      lineNo += 1;
      const lineTotal = Number(item.amount ?? item.line_total ?? 0);
      return [
        String(lineNo),
  ...
  const totalTax = Number(data.cgst ?? 0) + Number(data.sgst ?? 0) + Number(data.igst ?? 0);
  ...
  const taxRows: string[][] = isInterState
    ? [
        ['Taxable value', fmt(subtotal), 'IGST', fmt(data.igst ?? 0)],
        ['Round off', fmt(roundOff), 'Net payable', fmt(grandTotal)],
      ]
    : [
        ['Taxable value', fmt(subtotal), 'CGST', fmt(data.cgst ?? 0)],
        ['SGST', fmt(data.sgst ?? 0), 'Round off', fmt(roundOff)],
