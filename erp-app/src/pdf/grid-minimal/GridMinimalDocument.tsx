...
export default function GridMinimalDocument({
  vm,
  columns,
}: {
  vm: GridMinimalVM;
  ...
  const visibleColumns = React.useMemo(() => buildVisibleColumns(columns), [columns]);
          ...
          <View style={styles.totalsOuter}>
            <View style={styles.totalsInner}>
              <View style={styles.totalsRow}>
                <Text>Sub-total</Text>
                <Text>â\u20ac¹ {fmt(vm.totals.basicAmount)}</Text>
              </View>
              {vm.showTax && vm.totals.sgst !== undefined ? (
                <View style={styles.totalsRow}>
                  <Text>SGST</Text>
                  <Text>â\u20ac¹ {fmt(vm.totals.sgst)}</Text>
                </View>
              ) : null}
              {vm.showTax && vm.totals.cgst !== undefined ? (
                <View style={styles.totalsRow}>
                  <Text>CGST</Text>
                  <Text>â\u20ac¹ {fmt(vm.totals.cgst)}</Text>
                </View>
              ) : null}
              {vm.showTax && vm.totals.igst !== undefined && vm.totals.igst > 0 ? (
                <View style={styles.totalsRow}>
                  <Text>IGST</Text>
                  <Text>â\u20ac¹ {fmt(vm.totals.igst)}</Text>
                </View>
              ) : null}
              {vm.totals.roundOff !== undefined ? (
                <View style={styles.totalsRow}>
                  <Text>Round Off</Text>
                  <Text>
                    {vm.totals.roundOff > 0 ? '+' : ''}â\u20ac¹ {fmt(vm.totals.roundOff)}
                  </Text>
                </View>
              ) : null}
              <View style={[styles.totalsRow, styles.totalsNet]}>
                <Text>Net Value</Text>
                <Text>â\u20ac¹ {fmt(vm.totals.netValue)}</Text>
              </View>
            </View>
          </View>
