  ...
  return (
    ...
    >
      ...
      {col.due_date?.visible && (
        <div className="w-24 shrink-0 px-2 py-2.5 flex items-center justify-center">
          {task.due_date ? (
            <span
              className={cn(
                'inline-flex items-center justify-center px-1.5 py-0.5 rounded text-[10px] tabular-nums',
                getDueDateChipStyles(task.due_date, task.status)
              )}
            >
...
function getDueDateChipStyles(dueDateStr: string, status: TaskStatus): string {
  ...
  if (diffDays < 0) {
    return 'bg-rose-50 text-rose-600 border border-rose-200 font-semibold';
  } else if (diffDays <= 3) {
